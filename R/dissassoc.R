dissassoc<-function(diss,group , R=1000){
  ...
  ret$anova.table<-data.frame(SS=c(SCtot-SCres,SCres,SCtot), df=c(k-1,n-k,n-1),MSE=c((SCtot-SCres)/(k-1),SCres/(n-k),SCtot/(n-1)))
...
#  print(SCtot)
  if(R==1){
    ...
    ret$stat<-data.frame(PseudoF=vals[1],PseudoR2=vals[2],PseudoF_Pval=NA,PseudoT=vals[3],PseudoT_Pval=NA)
    ret$perms<-NA
  }
  else{
    bts<-boot(ind, internalBootstrapCompareGroups, R, sim="permutation", stype="i",
      dissmatrix=dissmatrix, indgrp=indgrp,SCtot=SCtot)
    ret$stat<-data.frame(PseudoF=bts$t0[1],PseudoR2=bts$t0[2], PseudoF_Pval=sum(bts$t[,1]>bts$t0[1])/bts$R,
...
hist.dissassoc<-function(x,test="PseudoF",breaks="FD",main=paste("Distribution of", test),xlab=test,pvalue.limit=NULL,freq=FALSE,...){
  ...
  if(x$R==1)stop("Cannot plot permutation test (none detected)")
  testbootorder<-order(x$perms$t[,ti],decreasing=TRUE)
  hist(x$perms$t[,ti],main=main, xlab=xlab,breaks=breaks,...)
  if(!is.null(pvalue.limit))abline(v=x$perms$t[testbootorder[round(pvalue.limit*x$R)],ti],col="blue")
