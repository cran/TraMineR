seqdiff<-function(seqdata,group, cmprange=c(0,1),seqdist_arg=list(method="LCS",norm=TRUE)){
...
#    sdist<-seqdist(method="OM",norm=TRUE, indel=1, sm=seqsubm(seqdata, method="CONSTANT",cval=2), seqdata= subseq[seqok,])
...
plot.seqdiff<-function(x,stat="PseudoR2",type="l",ylab=stat,xlab="",legendposition="top",ylim=NULL,...){
