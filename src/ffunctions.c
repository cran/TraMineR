...
void cLEVEN(int *seq1, int *seq2, double *param, double *scost, double *result) {
	...
	//Le vecteur param contient la taille des deux s√©quences (identique), le nombre d'√©tats, les indels
	...
	for(i=1;i<m;i++) {
		for(j=1;j<n;j++) {
			if(seq1[j-1] == seq2[i-1]) {
				//Rprintf("Seq1[%i] et Seq2[%i] sont Ègaux, %i == %i \n", j-1, i-1, seq1[j-1], seq2[i-1]);
				cost = 0;
			}
			else {
